...
Welcome back to your journey in testing React applications! Previously, we built a test using `ReactDOM`, manually creating DOM nodes, appending them to the body, cleaning up, and wrapping interactions in `flushSync()`. It worked—but it was a bit clunky.
...
RTL encourages best practices by helping you test components in a way that’s **closer to how users interact with them**. It removes boilerplate, automatically handles cleanup, and wraps everything in `flushSync()` behind the scenes.
