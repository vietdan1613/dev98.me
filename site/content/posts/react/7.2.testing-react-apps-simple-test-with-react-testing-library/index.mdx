...
Welcome back to your journey in testing React applications! Previously, we built a test using `ReactDOM`, manually creating DOM nodes, appending them to the body, cleaning up, and wrapping interactions in `act()`. It worked—but it was a bit clunky.
...
RTL encourages best practices by helping you test components in a way that’s **closer to how users interact with them**. It removes boilerplate, automatically handles cleanup, and wraps everything in `act()` behind the scenes.
...
✅ No need for `createRoot`, `act`, or `cleanup` — RTL handles that for you
✅ We use **screen queries** (like `getByText`) which resemble how users find things
✅ Assertions use **`toHaveTextContent`** from `@testing-library/jest-dom` — more expressive and helpful
✅ The test is shorter, clearer, and more maintainable

---

## 🔍 A Closer Look at `screen` and `fireEvent`
...
> 🧠 Tip: RTL also supports `userEvent`, which simulates interactions more realistically. We'll explore that later!

---

## 💯 Extra Credit: Use `@testing-library/jest-dom` for Better Assertions
...
* ✅ Gives better error messages when the assertion fails
* ✅ Automatically integrated in most RTL setups
...
🔗 [Elaboration & Feedback Form](https://ws.kcd.im/?ws=Testing%20React%20Applications%20%F0%9F%A7%90&e=02%3A%20simple%20test%20with%20React%20Testing%20Library&em=)
